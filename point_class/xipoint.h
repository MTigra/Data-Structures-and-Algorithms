...
public:
    /// Returns a raw pointer to the underlying array. Can be used only in RHV position
    /// The method is implemented as highly-effective inline function
    const Point* getRawPtr() const { return _arr; };

protected:

    /// Allocates a new array of size n, copies the first min(previous size, n)
    /// existing elements into it, and deallocates the old array.
    void resize(Uint n);
